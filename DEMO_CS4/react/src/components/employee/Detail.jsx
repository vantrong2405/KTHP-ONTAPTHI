...
export default function EmployeeDetail(props) {
  ...
  function get() {
    return Service.get(props.match.params.employeeId, props.match.params.departmentId, props.match.params.employeeTypeId, props.match.params.branchId, props.match.params.jobPositionId).then(response => {
          ...
          <form method="post">
            <div className="row">
              ...
              <div className="col-12">
                <Link className="btn btn-sm btn-secondary" to={Util.getRef('/employee')}>Back</Link>
                <Link className="btn btn-sm btn-primary" to={`/employee/edit/${employee.EmployeeID}/${employee.DepartmentID}/${employee.EmployeeTypeID}/${employee.BranchID}/${employee.JobPositionID}?ref=${encodeURIComponent(Util.getRef('/employee'))}`}>Edit</Link>
              </div>
            </div>
          </form>
        </div>
