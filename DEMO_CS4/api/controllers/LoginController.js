...
function getMenu(roles) {
  return menu
    .filter(
      e =>
        e.show &&
        (!e.roles || e.roles.split(',').some(role => roles.includes(role))),
    )
    .map(e => ({
      title: e.title,
      path: e.path,
...
exports.login = async (req, res, next) => {
  ...
  if (user && user.active && bcrypt.compare(req.body.password, user.password)) {
    let roles = await getUserRoles(user.id);
    let token = jwt.sign({ id: user.id, name: user.name, roles }, jwtSecret, {
      expiresIn: '1d',
    });
    console.log('Tao chạy xong r ');
  ...
  let message =
    user && !user.active ? 'User is disabled' : 'Invalid credentials';
